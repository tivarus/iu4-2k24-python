...
def tree(path: Path, depth: int, curr_lvl: int, init_symb: str = '', result: list = []) -> list:
    ...
    for symbol, path in zip(symbols, path_list):
        if (path.is_dir() == True):
            result.append(f'{Fore.WHITE}{init_symb}{symbol}{Fore.BLUE}{path.name}' )
            if(curr_lvl < depth) | (depth == 0):
                init_symb_next = init_symb + (Symbols.B.value if (symbol == Symbols.T.value) else Symbols.S.value ) 
                tree(path, depth, curr_lvl+1, init_symb_next, result)
        else:
            result.append(f'{Fore.WHITE}{init_symb}{symbol}{Fore.GREEN}{path.name}')
